...
filtered_df = None
pickle_file = 'path_to_databse_files'
...
def home():
    ...
    :return: zusina home page
    """
    return render_template('zusina.html')
...
def render_zusina2(df_show, warning=None):
    """
    Render zusina page with interactive pie chart and table.
    :param df_show: datfarme to display
    :param warning: possible warning message
    :return: zusina page rendered with interactive pie chart
    """
    ...
    max_label = names[0]
    return render_template('zusina2.html', labels=names, values=counts,
                           max_label=max_label, warning_message=warning)  # passing to values to zusina2.html
...
def show_pie_chart_database():
    ...
    global df_current
    df_current = df.copy()
    return render_zusina2(df_current)
...
def process_uploaded_data():
    ...
    global new_data
    global df_current
    user_path = './Application/uploaded_files/df_uploaded_' + session['user_id'] + '.pckl'
    if (not os.path.isfile(user_path)) or new_data:
        script_path = './Application/preprocessing.py'
...
def error2():
    ...
    df_current = df.copy()
    return render_zusina2(df_current, warning)    
...
def update_table():
    ...
    # elif topic selected and searched:
        ...
        if filtered_df is None or filtered_df.empty:
            return jsonify({'html': 'No results found.', 'labels': None, 'values': None, 'max_label': None, 'results_found': False})
        ...
        return jsonify({'html': html_table, 'labels': names, 'values': counts, 'max_label': max_label, 'results_found': True})
...
def reset_data():
    ...
    try:
        ...
        if os.path.exists('./Application/uploaded_files/df_uploaded_' + session['user_id'] + '.pckl'):
            os.remove('./Application/uploaded_files/df_uploaded_' + session['user_id'] + '.pckl')
...
def cleanup():
    ...
    """
    shutil.rmtree('./Application/uploaded_files/pdf/')
    for filename in os.listdir('./Application/uploaded_files/'):
        if filename.startswith('df_uploaded_'):
            file_path = os.path.join('./Application/uploaded_files/', filename)
            os.remove(file_path)
